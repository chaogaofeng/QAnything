...
def format_weather_data_now(data, place):
    forecast = data['now']
    formatted_data = f"\n{place}最新天气信息: \n"
    # 将预报时间转换为datetime对象
    forecast_time = datetime.strptime(forecast['obsTime'], '%Y-%m-%dT%H:%M%z')

    forecast_time_str = forecast_time.strftime('%Y-%m-%d %H:%M')
    # 将预报时间和当前时间的差值添加到输出中
    formatted_data += '预报时间: ' + forecast_time_str + '\n'
    formatted_data += '温度: ' + forecast['temp'] + '°C\n'
    formatted_data += '天气: ' + forecast['text'] + '\n'
    formatted_data += '风向: ' + forecast['windDir'] + '\n'
    formatted_data += '风速: ' + forecast['windSpeed'] + '级\n'
...
def weathercheck(model: ChatOpenAI, query: str):
